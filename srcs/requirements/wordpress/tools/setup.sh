...
for i in {1..60}; do
    ...
    echo "Waiting for MariaDB to be ready... ($i/60)"
    sleep 5
    if [ $i -eq 60 ]; then
...
# Configure PHP-FPM to listen on all interfaces (critical fix for the 502 error)
echo "Configuring PHP-FPM..."
sed -i 's|.*listen =.*|listen = 0.0.0.0:9000|g' /etc/php/*/fpm/pool.d/www.conf
echo "PHP-FPM configured to listen on 0.0.0.0:9000"

# Make sure all hosts can connect (remove any IP restrictions)
grep -q "listen.allowed_clients" /etc/php/*/fpm/pool.d/www.conf && \
    sed -i 's|.*listen.allowed_clients.*|listen.allowed_clients = any|g' /etc/php/*/fpm/pool.d/www.conf || \
    echo "listen.allowed_clients = any" >> /etc/php/*/fpm/pool.d/www.conf

# Create a status page for debugging
echo "Creating PHP-FPM status page..."
grep -q "pm.status_path" /etc/php/*/fpm/pool.d/www.conf && \
    sed -i 's|.*;pm.status_path.*|pm.status_path = /status|g' /etc/php/*/fpm/pool.d/www.conf || \
    echo "pm.status_path = /status" >> /etc/php/*/fpm/pool.d/www.conf

# Increase process limits for stability
sed -i 's|pm.max_children =.*|pm.max_children = 10|g' /etc/php/*/fpm/pool.d/www.conf
sed -i 's|pm.start_servers =.*|pm.start_servers = 3|g' /etc/php/*/fpm/pool.d/www.conf
sed -i 's|pm.min_spare_servers =.*|pm.min_spare_servers = 2|g' /etc/php/*/fpm/pool.d/www.conf
sed -i 's|pm.max_spare_servers =.*|pm.max_spare_servers = 5|g' /etc/php/*/fpm/pool.d/www.conf
...
if [ ! -f /var/www/html/wp-config.php ]; then
    ...
    wp theme install twentytwentyone --activate --allow-root || true
